...
- **Readability**: Softened left wipe → mid overlay → frosted blur under content (real win for CTAs/socials). Soft L/R dissolve needs working masks on wrappers (not `border-radius` alone); dual `mask-composite` can fail — nest single-axis masks. Keep mid scrims light so blur has tapestry detail.
